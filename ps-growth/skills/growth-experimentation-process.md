---
id: growth-experimentation-process
name: Growth Experimentation Process
domain: ps-growth
version: 1.0.0
---
# Growth Experimentation Process
**Purpose:** Apply growth experimentation process with the strategic depth, analytical rigor, and execution discipline that separates product teams that ship outcomes from product teams that ship features — where every decision is backed by evidence, connected to strategy, and measurable in impact.

## Why This Matters

Product growth is not marketing — it's product design for adoption and retention. Growth is the set of product decisions that determine how quickly users find value (activation), how reliably they return (retention), how naturally they bring others (referral), and how the revenue from each user increases over time (expansion). The best growth comes from the product itself — not from spending more on ads but from building a product that users can't stop using and can't stop telling others about.

For growth experimentation process specifically, this is one of the product decisions that compounds — get it right and everything downstream works better. Get it wrong and the team spends months building confidently in the wrong direction. The difference between high-performing product teams and struggling ones is usually 5-10 decisions like this one, made with frameworks instead of gut feeling.

## The Frameworks

The Growth Loop Framework: Growth loops are self-reinforcing cycles where the output of one cycle becomes the input of the next. Content loop: user creates content → content is indexed/shared → new users discover content → new users create content. Viral loop: user invites others → new users experience value → new users invite others. Data loop: more users → more data → better product → more users. Paid loop: revenue → marketing spend → new users → more revenue. The strongest products have multiple interlocking loops. The weakest depend on a single paid loop.

The Growth Equation: Growth = Acquisition × Activation × Retention × Revenue × Referral. Each factor is multiplicative — improving any one by 20% improves overall growth by 20%. But retention is the foundation: without retention, acquisition fills a leaky bucket. Without activation, acquired users never reach retention. The priority order: fix retention first, then activation, then acquisition, then referral, then revenue expansion. Most teams optimize acquisition first because it's visible. The best teams optimize retention first because it's leveraged.

Activation Framework: Define the 'aha moment' — the specific action that correlates with long-term retention. For Slack: sending 2,000 messages as a team. For Dropbox: placing a file in the synced folder. For Zoom: completing a 3+ person call. Measure: what % of signups reach the aha moment within the first session? The first 7 days? The first 30 days? The activation rate is the most important growth metric because it determines whether acquisition spend produces customers or just signups.
...
**Pre-PMF (0-100 users):** At this stage, growth experimentation process is about finding signal in noise. The sample size is too small for statistical significance. Optimize for speed of learning: 5-10 experiments per month, each producing a clear insight. The founder should be directly involved in every growth experimentation process decision.

**Post-PMF, scaling (100-10K users):** growth experimentation process must be systematized — turned from founder instinct into repeatable process. Document what works. Hire someone to own it. Build the metrics dashboard. The danger is premature optimization — don't over-engineer before validating at 3-5x current scale.

**At scale (10K+ users):** growth experimentation process is an operational discipline with an owner, a dashboard, a review cadence, and a continuous improvement process. Decisions are about optimization (improving efficiency) not exploration (finding what works). Quarter-over-quarter improvement is the goal.

## Case Studies

Dropbox's referral loop: 'Give 500MB, get 500MB' was the growth mechanic that took Dropbox from 100K to 4M users in 15 months. The mechanic worked because: (1) the reward was the product itself (storage), (2) both sides benefited, (3) the referral was naturally embedded in the product experience (sharing files), and (4) storage was the constraining resource that motivated referral. Duolingo's retention architecture: daily streaks, XP systems, leaderboards, and heart-based lives create multiple engagement hooks that together produce ~55% Day-30 retention for a free consumer app — among the highest in any category.

## Common Mistakes

The biggest mistake with growth experimentation process is treating it as a one-time decision instead of an ongoing discipline. Markets change. Users evolve. Competitors adjust. The team that reviews growth experimentation process quarterly outperforms the team that sets it and forgets it.
...
The third mistake is measurement avoidance. If you can't measure whether growth experimentation process is working, you can't improve it. Define the metric before executing. If the metric doesn't move, the approach isn't working — regardless of how good it looks in the deck.

## The Product Craft Dimension

growth experimentation process is ultimately a craft skill — it improves with deliberate practice. The progression:
