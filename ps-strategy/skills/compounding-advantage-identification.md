---
id: compounding-advantage-identification
name: Compounding Advantage Identification
domain: ps-strategy
version: 1.0.0
---
# Compounding Advantage Identification
**Purpose:** Apply compounding advantage identification with the strategic depth, analytical rigor, and execution discipline that separates product teams that ship outcomes from product teams that ship features — where every decision is backed by evidence, connected to strategy, and measurable in impact.
...
The North Star Metric: One metric that captures the core value the product delivers. Airbnb: nights booked. Spotify: time spent listening. Slack: messages sent. The North Star must: reflect genuine user value (not vanity), be influenceable by the team (not external market forces), lead to revenue (not just engagement), and be measurable weekly. The North Star is not a KPI — it's the compass. KPIs are the instruments. The compass tells you if you're heading north. The instruments tell you altitude, speed, and fuel.
...
**Pre-PMF (0-100 users):** At this stage, compounding advantage identification is about finding signal in noise. The sample size is too small for statistical significance. Optimize for speed of learning: 5-10 experiments per month, each producing a clear insight. The founder should be directly involved in every compounding advantage identification decision.

**Post-PMF, scaling (100-10K users):** compounding advantage identification must be systematized — turned from founder instinct into repeatable process. Document what works. Hire someone to own it. Build the metrics dashboard. The danger is premature optimization — don't over-engineer before validating at 3-5x current scale.

**At scale (10K+ users):** compounding advantage identification is an operational discipline with an owner, a dashboard, a review cadence, and a continuous improvement process. Decisions are about optimization (improving efficiency) not exploration (finding what works). Quarter-over-quarter improvement is the goal.
...
The biggest mistake with compounding advantage identification is treating it as a one-time decision instead of an ongoing discipline. Markets change. Users evolve. Competitors adjust. The team that reviews compounding advantage identification quarterly outperforms the team that sets it and forgets it.
...
The third mistake is measurement avoidance. If you can't measure whether compounding advantage identification is working, you can't improve it. Define the metric before executing. If the metric doesn't move, the approach isn't working — regardless of how good it looks in the deck.

## The Product Craft Dimension

compounding advantage identification is ultimately a craft skill — it improves with deliberate practice. The progression:
