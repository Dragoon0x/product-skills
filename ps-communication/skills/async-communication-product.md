---
id: async-communication-product
name: Async Communication Product
domain: ps-communication
version: 1.0.0
---
# Async Communication Product
**Purpose:** Apply async communication product with the strategic depth, analytical rigor, and execution discipline that separates product teams that ship outcomes from product teams that ship features — where every decision is backed by evidence, connected to strategy, and measurable in impact.
...
**Pre-PMF (0-100 users):** At this stage, async communication product is about finding signal in noise. The sample size is too small for statistical significance. Optimize for speed of learning: 5-10 experiments per month, each producing a clear insight. The founder should be directly involved in every async communication product decision.

**Post-PMF, scaling (100-10K users):** async communication product must be systematized — turned from founder instinct into repeatable process. Document what works. Hire someone to own it. Build the metrics dashboard. The danger is premature optimization — don't over-engineer before validating at 3-5x current scale.

**At scale (10K+ users):** async communication product is an operational discipline with an owner, a dashboard, a review cadence, and a continuous improvement process. Decisions are about optimization (improving efficiency) not exploration (finding what works). Quarter-over-quarter improvement is the goal.
...
The biggest mistake with async communication product is treating it as a one-time decision instead of an ongoing discipline. Markets change. Users evolve. Competitors adjust. The team that reviews async communication product quarterly outperforms the team that sets it and forgets it.
...
The third mistake is measurement avoidance. If you can't measure whether async communication product is working, you can't improve it. Define the metric before executing. If the metric doesn't move, the approach isn't working — regardless of how good it looks in the deck.

## The Product Craft Dimension

async communication product is ultimately a craft skill — it improves with deliberate practice. The progression:
