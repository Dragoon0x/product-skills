---
id: quantitative-research-product
name: Quantitative Research Product
domain: ps-discovery
version: 1.0.0
---
# Quantitative Research Product
**Purpose:** Apply quantitative research product with the strategic depth, analytical rigor, and execution discipline that separates product teams that ship outcomes from product teams that ship features — where every decision is backed by evidence, connected to strategy, and measurable in impact.
...
For quantitative research product specifically, this is one of the product decisions that compounds — get it right and everything downstream works better. Get it wrong and the team spends months building confidently in the wrong direction. The difference between high-performing product teams and struggling ones is usually 5-10 decisions like this one, made with frameworks instead of gut feeling.

## The Frameworks

Continuous Discovery (Teresa Torres): Talk to at least one customer per week, every week, as a team habit — not a quarterly research project. Use the Opportunity Solution Tree: Desired Outcome → Opportunities (user needs/pain points discovered through interviews) → Solutions (ideas that address opportunities) → Assumptions (things that must be true) → Experiments (cheap tests of assumptions). The tree makes the logic chain from user need to product decision visible and debatable.
...
**Pre-PMF (0-100 users):** At this stage, quantitative research product is about finding signal in noise. The sample size is too small for statistical significance. Optimize for speed of learning: 5-10 experiments per month, each producing a clear insight. The founder should be directly involved in every quantitative research product decision.

**Post-PMF, scaling (100-10K users):** quantitative research product must be systematized — turned from founder instinct into repeatable process. Document what works. Hire someone to own it. Build the metrics dashboard. The danger is premature optimization — don't over-engineer before validating at 3-5x current scale.

**At scale (10K+ users):** quantitative research product is an operational discipline with an owner, a dashboard, a review cadence, and a continuous improvement process. Decisions are about optimization (improving efficiency) not exploration (finding what works). Quarter-over-quarter improvement is the goal.
...
The biggest mistake with quantitative research product is treating it as a one-time decision instead of an ongoing discipline. Markets change. Users evolve. Competitors adjust. The team that reviews quantitative research product quarterly outperforms the team that sets it and forgets it.
...
The third mistake is measurement avoidance. If you can't measure whether quantitative research product is working, you can't improve it. Define the metric before executing. If the metric doesn't move, the approach isn't working — regardless of how good it looks in the deck.

## The Product Craft Dimension

quantitative research product is ultimately a craft skill — it improves with deliberate practice. The progression:
